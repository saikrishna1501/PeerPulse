...
import { deleteComment, editComment, loadComments, postComment } from "../../services/CommentsService";
...
const CommentsContainer = ({ blog }: Props) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            const fetchedComments = await loadComments(blog._id)
    ...
    const onCommentEdit = async (content: string, commentId: string | ObjectId | undefined) => {
        const response = await editComment(blog._id,commentId, {
            comment: content
        });
        if(response) {
            setCommentsList(commentsList.map((comment: any) => {
    ...
    const renderedComments = commentsList.map((comment: any) => {
        return (<Box sx={commentContainer}>
            <CommentCard key={comment._id} authorName={comment.authorName} authorId={comment.author} commentId={comment._id} content={comment.comment} avatarUrl={comment.avatarUrl} onCommentEdit={onCommentEdit} onCommentDelete={onCommentDelete}/>
        </Box>)
    })
        ...
        <Box sx={mainContainer}>
            ...
            <Box>
                <Box sx={{...commentContainer, border: 0}}>
                    <Typography mb={2} variant="h3" style={{ textAlign: "left", fontWeight: 600 }}>
            ...
            <Box>
                <Box sx={{...commentContainer, border: 0}}>
                    <CommentInput authorName={user.firstName + " " + user.lastName} avatarUrl={user.profilePic} onCommentPost={onCommentPost} />
                </Box>
            </Box>
